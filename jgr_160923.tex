...
The numerical procedures used in the EM-PIC simulation are described in \cite{???Omura}.  
In PIC simulation, both ions and electrons are treated as particles to 
...
The current density at each grid point is caluculated using the charge conservation method, 
which is based on the continuity equation for the charged particles \cite{??}.
To solve the evolution of electromagnetic fields in the simulation domain,
Maxwell's equations are solved by using the finite-difference time-domain method.
In Maxwell's equations, plasma dynamics are taken into account through
 the current density calculated with the paritcle motions.

Table \ref{fig:1} shows the parameters of the solar wind plasma and 
a magnetic dipole of Reiner Gamma which is a typical magnetic anomaly on the lunar surface.
...
%of $1.13 \times 10^{13} Am^2$ at the latitude of 7.5 degrees north, 
%the longitude of 58.4 degrees west, and 11.1 km under the near side of the moon.



According to the parameters, $r_\mathrm{iL}$ is approximately 120 km 
which is four times larger than $L$.  
...
\begin{figure}[t]
 \centering
%\noindent\includegraphics[width=12cm]{./Fig_1_bb-crop.pdf}
%\noindent\includegraphics[width=8cm]{./figures/Fig_1_bb-crop.pdf}
...
The values used for the upstream plasma representing the solar wind are as follows. 
Instead of using actual measured physical values, 
we used scaled ones to reduce the calculation time. 
We set the mass ratio between ion and electron to be $ m_\mathrm{i}/ m_\mathrm{e}$ = 100. 
...
the electron plasma frequency to 
...
Figure \ref{fig:2} shows a three-dimensional simulation model.
...
% -- we need to define the terminology shown in Fig 1 ---




In the positive $x$ direction, 
we have a plasma flow typically consisting of $5.37 \times 10^8 $ macroparticles in the simulation space. 
...
In this study, the IMF direction is along the positive $z$ direction.
The convective electric field $E_0$ in the plasma flow is along the positive $y$ direction. 

We have the lunar surface which is a $yz$ plane placed 
at the distance of $5.4L$ from the left boundary of the physical simulation region.
...
% -- need to insert the intensity of MA in terms of riL/L  --
In the simulations, 
...
%  --need to add
We allowed the simulations to iterate 
until the spatial variation of the plasma density reached a steady state. 
